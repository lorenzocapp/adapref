...
```{r}
source("adapref_INLA.R")
source("adapref_STAN.R")
source("utilities.R")
source("utilities_from_other_packages.R")
...
We now defines the various representation of the data that are needed for the estimation.
We also define the grid constituted by ngr points. 
```{r}
...
We can either to INLA or HMC posterior approximation.
First we show how to do INLA: the first line use the built in procedure to choose precisions hyperparameters of the fields; second line, it is the actually posterior approximation
...
Now, we show how to use HMC: first we define the parameters needed (par.G.H to keep, MCMC relevant quantities); then we use the in built procedure to determine the precision parameter of the field; we do the estimation (adapref_sampling). In this case we are fitting a GMRF-1 on both parameters. Lastly, we extract the output. 
